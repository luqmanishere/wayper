  ...
  config = mkMerge [
    (mkIf cfg.enable {
      ...
      xdg.configFile."wayper/config.toml".text = ''
        ...
        transitions_enabled = "${cfg.config.transitions.enable}"
        ${
          if (cfg.config.transitions.duration != null)
          then "transition_duration = ${cfg.config.transitions.duration}"
          else ""
        }
        ${
          if (cfg.config.transitions.fps != null)
          then "transition_fps = ${cfg.config.transitions.fps}"
          else ""
        }
        ...
        ${builtins.concatStringsSep "\n" (map (monitor: ''
            ...
            transitions_enabled = "${monitor.transitions.enable}"
            ${
              if (monitor.transitions.duration != null)
              then "transition_duration = ${monitor.transitions.duration}"
              else ""
            }
            ${
              if (monitor.transitions.fps != null)
              then "transition_fps = ${monitor.transitions.fps}"
              else ""
            }
            ${
              if (monitor.transitions.type != null)
              then "transition_type = ${monitor.transitions.type}"
              else ""
            }
          '')
